      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {loading ? (
          ...
          <div className="border border-white/10 rounded-2xl bg-white/[0.02] p-12 text-center">
            <p className="text-white/40 mb-4">No goals yet</p>
            <Link href="/projects/goals/new">
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    {goal.description && <p className="text-white/40 text-sm mb-2">{goal.description}</p>}
                    <p className="text-white/70 text-sm">
                      <span className="text-white/40">Next: </span>
                      {goal.next_action || <span className="text-white/30 italic">not set</span>}
                    </p>
                  </div>
                  <div className="flex gap-2">
                    <Link
                      href={`/projects/goals/${goal.id}/edit`}
                    ...
                    {goal.status === 'active' ? (
                      <>
                        <button
                          onClick={() => setStatus(goal.id, 'done')}
                          className="p-2 rounded-lg hover:bg-white/5 transition-colors"
                          title="Mark done"
                        >
